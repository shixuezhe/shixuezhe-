...
def manage_job(company_id):
    ...
    return render_template('company/manage_job.html',pagination=pagination)
...
def edit_job(company_id):
    company = Company.query.filter_by(users_id=company_id).first()
    job = Job.query.get_or_404(company.id)
    form = JobForm(obj=job)
    if form.validate_on_submit():
        form.update_job()
        flash('职位更新成功','success')
        return redirect(url_for('company.manage_job'))
    return render_template('company/edit_job.html',form=form,company=company)
